    ...
    if (rawGamepads) {
      ...
      for (var i = 0; i < rawGamepads.length; i++) {
        ...
        if (
          rawGamepads[i] &&
          controllerRebinds != "" &&
          typeof controllerRebinds.mapping != "undefined" &&
          controllerRebinds.mapping.length > 0
        ) {
          ...
          for (var m = 0; m < controllerRebinds.mapping.length; m++) {
            ...
            if (
              tType == "buttons" &&
              typeof remapObj[tType][tMap] == "undefined"
            ) {
              remapObj[tType][tMap] = {};
            }
            ...
            function dpadPOV(stickObj) {
              ...
              function isWithinRange(val, target) {
                if (val >= target - 0.001 && val <= target + 0.001) return true;
                else return false;
              }
              ...
              switch (typeof setValue) {
                ...
                case "Number":
                  if (stickObj.targetType == "axes") {
                    ...
                    remapObj[stickObj.targetType][stickObj.target].value =
                      setValue;
                  }
                  break;
                case "object":
                case "Object":
                  if (stickObj.targetType == "axes") {
                    remapObj[stickObj.targetType][stickObj.target] = makeAxis(
                      stickToButton(stickObj.positive),
                      stickToButton(stickObj.negative)
                    );
                  } else {
                    remapObj[stickObj.targetType][stickObj.target].value =
                      stickToButton(stickObj);
                  }
                  break;
                default:
              }
